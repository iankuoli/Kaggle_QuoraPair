...
STAMP = 'blstm_ns_%d_%d_%.2f_%.2f' % (num_lstm, num_dense, rate_drop_lstm, rate_drop_dense)
...
#
# input for Bidirectional LSTM
# ----------------------------------------------------------------------------
train_data_1_rev = pad_sequences(train_sequences_1, maxlen=MAX_SEQUENCE_LENGTH, padding='post')
train_data_2_rev = pad_sequences(train_sequences_2, maxlen=MAX_SEQUENCE_LENGTH, padding='post')
data_1_train_rev = np.vstack((train_data_1_rev, train_data_2_rev))
data_2_train_rev = np.vstack((train_data_2_rev, train_data_1_rev))

valid_data_1_rev = pad_sequences(valid_sequences_1, maxlen=MAX_SEQUENCE_LENGTH, padding='post')
valid_data_2_rev = pad_sequences(valid_sequences_2, maxlen=MAX_SEQUENCE_LENGTH, padding='post')
data_1_valid_rev = np.vstack((valid_data_1_rev, valid_data_2_rev))
data_2_valid_rev = np.vstack((valid_data_2_rev, valid_data_1_rev))

test_data_1_rev = pad_sequences(test_sequences_1, maxlen=MAX_SEQUENCE_LENGTH, padding='post')
test_data_2_rev = pad_sequences(test_sequences_2, maxlen=MAX_SEQUENCE_LENGTH, padding='post')
...
sequence_1_rev_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
sequence_2_rev_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
...
embedded_sequences_1_rev = embedding_layer(sequence_1_rev_input)
x1r = lstm_layer(embedded_sequences_1_rev)

x1 = concatenate([x1, x1r])
...
embedded_sequences_2_rev = embedding_layer(sequence_2_rev_input)
y1r = lstm_layer(embedded_sequences_2_rev)

y1 = concatenate([y1, y1r])
...
# ----------------------------------------------------------------------------
# rate_drop_ns_w2v = 0.5
rate_drop_ns_w2v = rate_drop_dense

embedded_sequences_3 = embedding_layer_ns(sequence_1_input)
d1 = Dropout(rate_drop_ns_w2v)(embedded_sequences_3)
x2 = lstm_layer(d1)

embedded_sequences_3_rev = embedding_layer_ns(sequence_1_rev_input)
d1 = Dropout(rate_drop_ns_w2v)(embedded_sequences_3_rev)
x2r = lstm_layer(d1)

x2 = concatenate([x2, x2r])
x2 = BatchNormalization()(x2)
x2 = Dense(num_dense, activation=act)(x2)
x2 = Dropout(rate_drop_dense)(x2)

embedded_sequences_4 = embedding_layer_ns(sequence_2_input)
d2 = Dropout(rate_drop_ns_w2v)(embedded_sequences_4)
y2 = lstm_layer(d2)

embedded_sequences_4_rev = embedding_layer_ns(sequence_2_rev_input)
d2 = Dropout(rate_drop_ns_w2v)(embedded_sequences_4_rev)
y2r = lstm_layer(d2)

y2 = concatenate([y2, y2r])
y2 = BatchNormalization()(y2)
y2 = Dense(num_dense, activation=act)(y2)
y2 = Dropout(rate_drop_dense)(y2)
...
# Train the model
# ----------------------------------------------------------------------------
model = Model(inputs=[sequence_1_input, sequence_2_input, sequence_1_rev_input, sequence_2_rev_input],
              outputs=preds)
...
hist = model.fit([data_1_train, data_2_train, data_1_train_rev, data_2_train_rev],
                 labels_train,
                 validation_data=([data_1_valid, data_2_valid, data_1_valid_rev, data_2_valid_rev], labels_valid, weight_val),
...
preds = model.predict([test_data_1, test_data_2, test_data_1_rev, test_data_2_rev], batch_size=4096, verbose=1)
preds += model.predict([test_data_2, test_data_1, test_data_2_rev, test_data_1_rev], batch_size=4096, verbose=1)
preds /= 2
